  ...
  <body style="align-items: center">
    <script>
      ...
      function pageing(data, n, mode) {
        ...
        data[0].pagepart % 8 === 0
          ? $("#endpage").val(parseInt(data[0].pagepart / 8))
          : $("#endpage").val(parseInt(data[0].pagepart / 8 + 1));
        ...
        if (start > 10) {
          first =
            first +
            "<input class='btn btn-outline-secondary' type='button' onclick='changePage(1,1)' value='처음' /><input class='btn btn-outline-secondary' type='button' onclick='changePage(" +
            (start - 1) +
            "," +
            parseInt(start / 10) +
            "," +
        ...
        if (end >= lastpage) {
          //총페이지 보다 마지막페이지가 큰 경우 마지막 페이지는 총 페이지 수와 같아야 한다
          end = lastpage;
          for (start; start <= end; start++) {
            if (start != end) {
              pagestr =
                pagestr +
                "<input class='btn btn-outline-secondary' id='inputremote' type='button' onclick='changePage(" +
                start +
                "," +
                (parseInt(start / 10) + 1) +
                "," +
          ...
          //총 페이지보다 마지막 페이지가 작은 경우 10개를 추출해야 한다
          for (start; start <= end; start++) {
            if (start !== end) {
              pagestr =
                pagestr +
        ...
        if (end < reallastpage) {
          ...
          last =
            last +
            "<input class='btn btn-outline-secondary' type='button' onclick='changePage(" +
            start +
            "," +
          ...
          for (i in data) {
            ...
            if (count < 5) {
              var row = $(".tableRow").append(
                $("<td/>").append(
                  "<div class='card' style='min-width: 275px ;' ><a href='missionDetail.jsp?mission_no=" +
                    mission_no +
                    "'> <div class='card-header' style='min-height: 200px ;' ;> <img style='width: 250px ; height: 142px' src='http://localhost/api/image/" +
                    mission_no +
                    "'></div></a><biv class='card-body'>미션명 : " +
                    mission_name +
                    "<br>기간 : " +
                    start_date +
                    "~" +
      ...
      function changePage(nowpage, start, mode) {
        ...
        if (mode == search) {
          ...
          var fetchurl =
            "http://localhost/api/search?first=" +
            first +
            "&second=" +
            second +
            "&pageNum=" +
      ...
      function searchchange() {
        ...
        if (optionval === "missionname") {
          var changehtml =
            "<input type='text' id='firstval'> <input type='button' id='submitbutton' value='찾기' onclick='searchstart()'> ";
          $("#changearea").html(changehtml);
        }

        if (optionval === "term") {
          var changehtml =
            "<input type='date' id='firstval' >~<input type='date' id='secondval'> <input type='button' id='submitbutton' value='찾기' onclick='searchstart()'> ";
          $("#changearea").html(changehtml);
        }
      }
        ...
        <div class="row">
          ...
          <div
            ...
            <div id="listTable">
              <div
                class="tableRow"
                style="max-height: 800px; min-width: 600px"></div>
              <div
                class="tableRow2"
                style="max-height: 800px; min-width: 600px"></div>
            </div>
            ...
            <spen id="search">
              <select id="searchoptions" onchange="searchchange()">
                <option id="" value="">검색방법을 선택하세요</option>
                <option id="term" value="term">기간</option>
                <option id="missionname" value="missionname">미션명</option>
              </select>
            </spen>
            <span id="changearea"> </span>
          </div>

          <div id="page-button" align="center"></div>
